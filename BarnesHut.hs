...
forcaGravitacional m1 m2 r = --força gravitacional entre dois corpos
	if r == 0  then  0 
...
--estrutura QUadtree - generalização de uma arvore binaria
...
-- necessario para usar função force na arvore pois rpar apenas avalia ate primeiro construtor
...
--calculo das força individuais para o caso onde uma folha da arvore tem mais de um corpo-caso especial para não dar loop
...
--somatorio das forças calculadas na arvore na arvore  
...
somaForcas (Nodo m coordenadas limites (Empty _) (Empty _) (Empty _) (Empty _)) corpo = 
	if (pertence limites corpo)== True then (0,0)
...
--realiza o calculo da força gravitacional para os corpos
...
--verifica se um corpo pertence ao quadranta a um nodo da arvore
...
--quantidade de movimentos relizados
...
-- realiza um movimento de todos os corpos em paralelo
...
--um unico movimento de todos os corpos
...
--calcula a força para todos os corpos
...
--particiona para depois paralelizar 																		
...
--geração randomica dos corpos no espaço
